...
void biubussplit()
{
    ...
    biumerge_o_rsp_read  = biusplit_i_bus_rsp_read ;
    ...
    //    biutransacFIFO_wid =  biutransacFIFO_wid | ((biusplit_o_bus_cmd_valid[i] & biusplit_o_bus_cmd_ready[i]) ? i : 0);
        biutransacFIFO_wid = biutransacFIFO_wid | (biusplit_o_bus_cmd_valid[i] ? i :0);
    }
    ...
    for (i=0;i<BIUSPLITTARGETNUM;i++)
    {
        biusplit_i_bus_rsp_read  = biusplit_i_bus_rsp_read  | (biutransacFIFO_rid==i ? biusplit_o_bus_rsp_read[i]  : 0) ;
    ...
    biusplit_o_bus_rsp_read[0]  = plic_rsp_read;
    ...
    biusplit_o_bus_rsp_read[1]  = clint_rsp_read;
    ...
    biusplit_o_bus_rsp_read[2]  = peripheral_rsp_read;
    ...
    //
    biusplit_i_bus_cmd_ready=biusplit_o_bus_cmd_ready[0]|biusplit_o_bus_cmd_ready[1]|biusplit_o_bus_cmd_ready[2];
}